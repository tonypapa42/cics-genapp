...
Define Transaction(SSP6) Group(GENASAT)
       Program(LGTESTP6) TaskDataLoc(Any)   TaskDataKey(User)
...
Define Program(LGTESTP4) Group(GENASAP)
       ...
       Language(Cobol)   DataLocation(Any)   Execkey(User)
Define Program(LGTESTP6) Group(GENASAP)
       Description(Solution Pet Policy Menu)
       Language(Cobol)   DataLocation(Any)   Execkey(User)
